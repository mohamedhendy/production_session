<template>
    ...
    <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-2">
        <div class="form-group">
            <label class="capitalize"> {{ $t('locationName') }} </label>
            <div>
                <div class="">
                    <input type="text" v-model="contentObject.location_name" class="form-input" />
                </div>
            </div>
        </div>
        <div class="form-group">
            <label class="capitalize"> {{ $t('responsiblePerson') }} </label>
            <div>
                <div class="">
...
<script>
    ...
    export default {
        ...
        mounted() {
            if (this.isEditable) {
                this.contentObject = this.locationVal;
                this.defaultContractObj = JSON.stringify({
                    id: this.locationVal.legal_form.id,
                    name_en: this.locationVal.legal_form.name_en,
                    name_ar: this.locationVal.legal_form.name_ar,
                    is_active: this.locationVal.legal_form.is_active,
                });
                this.getContractContent(this.locationVal.legal_form.id);
            } else {
        ...
        watch: {
            ...
            locationVal() {
                this.contentObject = this.locationVal;
                this.contentObject.legal_form_id = this.locationVal.legal_form.id;
                this.defaultContractObj = JSON.stringify({
                    id: this.locationVal.legal_form.id,
                    name_en: this.locationVal.legal_form.name_en,
                    name_ar: this.locationVal.legal_form.name_ar,
            ...
            contentObject: {
                handler() {
                    this.$emit('addLocation', this.contentObject, this.indexNumber);
                },
                deep: true,
            },
        },
        methods: {
            deleteObj() {
                this.$emit('delete', this.indexNumber, 'location');
            },
            setSignatureWidth() {
                setTimeout(() => {
            ...
            filesSelected(e) {
                ...
                this.clear();
                this.contentObject.contract_image = this.contentObject.contract_image.file;
            },
            viewValue() {
                var _this = this;
