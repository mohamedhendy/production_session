<template>
    ...
    <div class="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-2">
        <div class="form-group">
            <label class="capitalize"> {{ $t('modelName') }} </label>
            <div>
                <div class="">
        ...
        <div class="form-group">
            ...
            <div>
                <div class="">
                    <input type="test" v-model="contentObject.identity_number" class="form-input" />
                </div>
            </div>
        </div>
        <div class="form-group">
            <label class="capitalize"> {{ $t('profilePicture') }} </label>
            <div class="">
                <vue-file-agent
                    @select="filesSelectedModelImg($event)"
                   
                    v-model="contentObject.model_image"
                ></vue-file-agent>
                <img
                    class="w-100"
                    style="object-fit: contain"
                    v-if="typeof contentObject.model_image == 'string' && contentObject.model_image != ''"
                    :src="cdnPath + contentObject.model_image"
                />
            </div>
        </div>
                                    ...
                                    <div class="p-5" ref="contract" :class="ConfirmDefaultContract ? 'activeSign' : ''">
                                        <div class="form-group">
                                            ...
                                            <div>
                                                <div class="">
                                                    <select
                                                        v-model="defaultContractObj"
                                                        @change="changeContract"
                                                        :placeholder="$t('ContractForms')"
...
<script>
    ...
    export default {
        ...
        emits:  ["addRepresentative", "delete"],
        data() {
            return {
        ...
        mounted() {
            if (this.isEditable) {
                this.contentObject = this.representativeVal;
                this.defaultContractObj = JSON.stringify({
                    id: this.representativeVal.legal_form.id,
                    name_en: this.representativeVal.legal_form.name_en,
                    name_ar: this.representativeVal.legal_form.name_ar,
                    is_active: this.representativeVal.legal_form.is_active,
                });
                this.getContractContent(this.representativeVal.legal_form.id);
            } else {
        ...
        watch: {
            ...
            representativeVal() {
                this.contentObject = this.representativeVal;
                this.contentObject.legal_form_id = this.representativeVal.legal_form.id;
                this.defaultContractObj = JSON.stringify({
                    id: this.representativeVal.legal_form.id,
                    name_en: this.representativeVal.legal_form.name_en,
                    name_ar: this.representativeVal.legal_form.name_ar,
            ...
            contentObject: {
                handler() {
                    this.$emit('addRepresentative', this.contentObject, this.indexNumber);
                },
                deep: true,
            },
        },
        methods: {
            deleteObj() {
                this.$emit('delete', this.indexNumber, 'representative');
            },
            setSignatureWidth() {
                setTimeout(() => {
            ...
            filesSelectedModelImg(e) {
                this.contentObject.model_image = e[0].file;
            },
            viewValue() {
                var _this = this;
